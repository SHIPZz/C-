...
inline bool simple_iteration(double (*func)(double), double (*g)(double), double x0, double tol, int max_iter, double &root) {
    int iter = 0;
    double x = x0;
    while (iter < max_iter) {
        double x_next = g(x);
        if (fabs(x_next - x) < tol) { // Проверка условия остановки
            root = x_next;
            return true;
        }
        x = x_next;
